...
library Util {
    ...
    //frontend must ensure that only valid cities are entered
    function getCitiesIdx(bytes32[] memory _cities, uint _mapnumber) internal pure returns(uint[] memory) {
        uint[] memory citiesIdx = new uint[](_cities.length);
        for(uint i = 0; i < _cities.length; i++) {
            /*if(_mapnumber == 24){
                citiesIdx[i] = getCityIdx(_cities[i], Maps.getMap24());
            }*/

            if(_mapnumber == 30){
                citiesIdx[i] = getCityIdx(_cities[i], Maps.getMap30());
            }

            if(_mapnumber == 70){
                citiesIdx[i] = getCityIdx(_cities[i], Maps.getMap70());
            }
        }
    ...
    function getCityIdx(bytes32 city, bytes32[] memory map) internal pure returns(uint){
